...
from utils.utils_view_3d import utils_view_3D,imgToBitmap
    ...
    def Onclick_visualization(self,event):
        if event.GetEventObject() == self._caidan1:
            ...
            if self.filename_visualization == None:
                ...
                print("点击了三视图映像！")
                img = nib.load(self.filename_visualization)
                print (img.dataobj.shape)

                img_arr_sp = img.dataobj[70,:,:]
                img_arr_gz = img.dataobj[:,:,250]
                img_arr_sz = img.dataobj[:,250,:]

                self.static_text = wx.StaticText(self.panel_visualization, label="横截面", pos=(280, 15), size=(100, 30))
                self._img_arr_sp = imgToBitmap(img_arr_sp,'sp')
                wx.StaticBitmap(self.panel_visualization, -1, self._img_arr_sp,pos=(175,40))

                self.static_text = wx.StaticText(self.panel_visualization, label="冠状图", pos=(510, 15), size=(100, 30))
                self._img_arr_gz = imgToBitmap(img_arr_gz,'rotate')
                wx.StaticBitmap(self.panel_visualization, -1, self._img_arr_gz,pos=(505,40))

                self.static_text = wx.StaticText(self.panel_visualization, label="矢状图", pos=(630, 15), size=(100, 30))
                self._img_arr_sz = imgToBitmap(img_arr_sz,'rotate')
                wx.StaticBitmap(self.panel_visualization, -1, self._img_arr_sz,pos=(625,40))

                self._visualization_onlyRead_text_exe = wx.TextCtrl(self.panel_visualization, value="待读入...   默认切片位置为: 70,250,250",style=wx.TE_READONLY, pos=(180, 300), size=(250, 25))

                dlg = wx.TextEntryDialog(None, "请在下面文本框中输入位置（Z,X,Y）:", "切片位置", "70,250,250")
                if dlg.ShowModal() == wx.ID_OK:
                    self.gapPos = dlg.GetValue()
                    self._visualization_onlyRead_text_exe.Label = self.gapPos
                dlg.Destroy()
        elif event.GetEventObject() == self._caidan3:
            if self.filename_visualization == None:
                wx.MessageBox('请导入数据！')
            else:
