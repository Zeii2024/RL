...
class RailEnv(Environment):
    ...
    The valid actions in the environment are:  """#五种动作：
    """
    ...
    - invalid_action_penalty = 0                                        #无效动作惩罚为0
    - step_penalty = -alpha                                             #每走一步惩罚alpha
    - global_reward = beta                                              #全局奖励bata
    - epsilon = avoid rounding errors                                   #
    - stop_penalty = 0  # penalty for stopping a moving agent           #停止操作惩罚为0
    - start_penalty = 0  # penalty for starting a stopped agent         #启动操作惩罚为0

    Stochastic malfunctioning of trains:                                #经常停车会出故障（主动或被动停车都算）
    ...
    complexity managable.   """ #每次停车都有一定概率会出故障，满足一定概率的泊松过程，不是所有车都会受故障影响，以此来降低复杂度
    """
    ...
    def __init__(self,
        ...
        self.stop_penalty = 0 * self.alpha # penalty for stopping a moving agent
        ...
        self.done_already = dict.fromkeys(list(range(number_of_agents)), False)
        ...
        self.num_resets = 0
        self.distance_map = DistanceMap(self.agents, self.height, self.width)  # distance_map

        self.action_space = [5]

        #######################
        self.agents_reset_paras = dict()
        ########################
    ...
    def compute_max_episode_steps(width: int, height: int, ratio_nr_agents_to_nr_cities: float = 20.0) -> int:
        ...
        """
        timedelay_factor = 4
        alpha = 2                      # = 4 * 2 * (width + height + num_agents / num_cities)
    ...
    def reset(self, regenerate_rail: bool = True, regenerate_schedule: bool = True, activate_agents: bool = False,
        ...
        if regenerate_rail or self.rail is None:
            ...
            ######################
            self.agents_reset_paras.update({"rail": self.rail, "optionals":optionals})
            ######################
        ...
        if regenerate_schedule or regenerate_rail or self.get_num_agents() == 0:
            ...
            ########################
            self.agents_reset_paras.update({"agents_hints": agents_hints, "num_resets":self.num_resets})
            self.agents_reset_paras.update({"number_of_agents":self.number_of_agents})
            ###############################################################
            # 下面两行，给schedule加上了self
            self.schedule = self.schedule_generator(self.rail, self.number_of_agents, agents_hints, self.num_resets)
            self.agents = EnvAgent.from_schedule(self.schedule)
            # if self.agents.position:
            # print("env.self.agents:",self.agents)
        ...
        # Return the new observation vectors for each agent active_agents
...
reset
        """
        # 修改rewards为单步的rewards
        ##########################################
        #计算总步数，关掉了
        # self._elapsed_steps += 1   #步数超过最大值会重置agents
        ##########################################
        ...
        if self.dones["__all__"]:
            ...
            for i_agent, agent in enumerate(self.agents):
                ###########################################
                # 这里的global_reward改成了complete_reward
                if not self.done_already[i_agent]:
                    self.rewards_dict[i_agent] = self.complete_reward + self.step_penalty # 每一个都是global_reward
        ...
        for i_agent, agent in enumerate(self.agents):
            ...
            ############################################ fifth
            # 增加单个agent到达终点的奖励
            if self.dones[i_agent] and not self.done_already[i_agent]:
                self.rewards_dict[i_agent] = self.target_reward
                # 加过一次终点奖励后，把done_already设为True
                self.done_already[i_agent] = True
            ###########################################
            # Induce malfunction before we do a step, thus a broken agent can't move in this step
            # 每个agent都有一定机率 break down ！！
            # self._break_agent(agent)

            # Perform step on the agent
            # step_agent会得到一个step_penalty，值为-1
            # print("_step_agent")
        ...
        if have_all_agents_ended:
            ...
            ###########################################
            # 这里的global_reward改成了complete_reward
            self.rewards_dict = {i: self.complete_reward+self.rewards_dict[i] for i in range(self.get_num_agents())}
        ''' # 去掉了
    ...
    def _step_agent(self, i_agent, action: Optional[RailEnvActions] = None):
        ...
        if np.isclose(agent.speed_data['position_fraction'], 0.0, rtol=1e-03):  #动作执行逻辑单元！！！速度约为0时
            ...
            if agent.moving:
                ...
                # print("new cell valid: ",new_cell_valid)
                ...
                if all([new_cell_valid, transition_valid]):
                    ...
                    if (action == RailEnvActions.MOVE_LEFT or action == RailEnvActions.MOVE_RIGHT):
                        ...
                        if all([new_cell_valid, transition_valid]):
                            ...
                            _action_stored = True
                            # 将动作left和right后，要加上一个改动作的惩罚，避免下次还会出现left或right
        ...
        if agent.moving:
            ...
            if agent.speed_data['position_fraction'] > 1.0 or np.isclose(agent.speed_data['position_fraction'], 1.0,
                ...
                # print("cell is free: ", cell_free)
                # print("new_cell_valid:",new_cell_valid,"\nnew_position:",new_position)
                # print("transition_valid:",transition_valid)
                if cell_free:
    ...
    def _move_agent_to_new_position(self, agent: EnvAgent, new_position: IntVector2D):
        ...
        self.agent_positions[agent.position] = agent.handle
        # print("move done")
        # print("agent.handle_position:",self.agent_positions[agent.position])
...
reset
    ...
    def reset_agent(self, a, agents_init_info):
        # self.agents = EnvAgent.from_schedule(self.schedule)
        # self.restart_agents()
        ...
        # set position = initial_position
        self.agents[a].initial_position = agents_init_info["initial_position"]
        self.agents[a].position = self.agents[a].initial_position
        self.agents[a].speed_data = agents_init_info['speed_data']
        self.agents[a].target = agents_init_info['target']
        self.agents[a].direction = agents_init_info['direction']
        self.agents[a].handle = agents_init_info['handle']
        # 把所有position重置为-1，即可用
        self.agent_positions = np.zeros((self.height, self.width), dtype=int) - 1
        
        #######################################
        # Induce malfunctions
        if self.cell_free(self.agents[a].initial_position):
            self._set_agent_to_initial_position(self.agents[a], self.agents[a].initial_position)
            self.agents[a].status = RailAgentStatus.ACTIVE
        self.agents[a].moving = True
        ######################################
        # self.agent_positions[self.agents[a].position] = self.agents[a].handle
        ######################################
        # for a in self.agents:
            # print("after:", a.position,a.target)

        self.dones[a] = False
        self.dones['__all__'] = False
        self.done_already[a] = False
        '''
        print("reset_agent_target:", self.agents[a].target)
        print("reset_agent_init_position:",self.agents[a].initial_position)
        print("reset_agent_position:",self.agents[a].position)
        time.sleep(3)
        '''
        # print("################### reset agent")
        # Reset the state of the observation builder with the new environment
